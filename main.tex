...
\begin{enumerate}
	...
	Come risolvere il problema delle chiavi troppo lunghe? 
\end{enumerate}

\section{Stream ciphers}
\begin{itemize}
	\item seed $k$ on $\{0,1\}^s$, the new key
	\item \textit{key stream} on $\{0,1\}^n$
	\item pseudo-random number generator $G : \{0,1\}^s \rightarrow \{0,1\}^n, s \ll n$
	\item $c = E(G(k), m) = G(k) \oplus m$
	\item $m = D(G(k), c) = G(k) \oplus c$
\end{itemize}

The cipher is not perfect for Shannon's Theorem. 

\subsection{Computational security}
There exist no efficient algorithm to distinguish PRNG output from a TRG output.

The cipher is computational secure if and only if:
\begin{itemize}
	\item PRNG is \textbf{good statistic}, i.e. it must be able to generate a key stream that looks like that one generated by an uniform random process
	\item PRNG is \textbf{unpredictable}
\end{itemize}
...
It produces good statistic streams but it is predictable since it is linear.

The key could be the pair $(A,B)$ which can be discovered if an adversary knows the first bytes of the plaintext and ciphertext.
...
\end{itemize}
The KTP attack has a complexity of $O(2^{17})$: if an adversary knows the first 20 bytes of plaintext and ciphertext it can get the first 20 bytes of the keystream. Then it can compute the 25-bit LFSR making the difference between the 17-bit LFSR random seed and the computed keystream. Then it has to check if the 25-bit LFSR output is consistent with the polynomial, that is known.

\subsection{WEP}
\begin{itemize}
	\item pre-shared $k$
	\item $iv$ on $\{0,1\}^{24}$ written in the standard, required to be fresh
	\item $ks = RC4(k||iv)$, where RC4 is the PRNG
	\item $m = m||crc(m)$
	\item $c = m \oplus ks$
\end{itemize}
...
\begin{enumerate}[label=\alph*.]
	\item $ks$ only depends on $iv$. After $2^{24}$ frames it repeats ciclically and $ks$ is reused
	\item use of CRC instead of hash function cheated by perturbation
	\item use of related keys $k||iv, iv = 0, 1, 2, ..., 2^{24}-1$. RC4 is good unless related keys are used.
\end{enumerate}

\section{Block ciphers}
Plaintext divided in blocks of fixed length $n$:
...
\end{itemize}
E is a \textbf{permutation}.

A perfect block cipher implements all possible permutations $2^n!$
